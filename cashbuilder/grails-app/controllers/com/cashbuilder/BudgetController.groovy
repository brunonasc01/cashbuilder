...
class BudgetController {
    ...
    def index() {
		...
		def bugetMonth = BudgetMonth.findByMonthAndBudget(month,budget)

		def months = BudgetMonth.findAllByBudget(budget)

		def listEntradas = budgetService.getBudgetData(user, bugetMonth, true)
		...
		balanceBox.plannedBalance = budgetService.getBudgetedBalance(bugetMonth)
		balanceBox.actualBalance = budgetService.getBalance(bugetMonth,user)
